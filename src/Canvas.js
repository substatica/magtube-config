...
const Canvas = props => {
    ...
    useEffect(() => {
        ...
        const drawLineWithStartLengthAngleOffset = (ctx, x1, y1, l1, d1, o1=0, t1=15, stroke = 'white') => {
            ...
            drawLine(ctx, x2, y2, x3, y3, stroke, t1);
            return {
                X: x2,
                Y: y2
            }
        ...
        const getPointWithOffsetStartAngle = (ctx, x1, y1, o1, d1) => {
            const x2 = x1 + o1 * Math.cos(Math.PI * d1 / 180.0);
            const y2 = y1 + o1 * Math.sin(Math.PI * d1 / 180.0);
            return {
                X: x2,
        ...
        const getOffsetPoint = (ctx, offset) => {
            if(offset < MagTubeConfig.StockSegementLengths[0]) {
                return getPointWithOffsetStartAngle(ctx, MagTubeConfig.StockSegmentPositions[0].X, MagTubeConfig.StockSegmentPositions[0].Y, offset, props.StockPivotAngles[0]);
            } else if (offset < MagTubeConfig.StockSegementLengths[0] + MagTubeConfig.StockSegementLengths[1]) {
                return getPointWithOffsetStartAngle(ctx, MagTubeConfig.StockSegmentPositions[1].X, MagTubeConfig.StockSegmentPositions[1].Y, offset - MagTubeConfig.StockSegementLengths[0], props.StockPivotAngles[1] + props.StockPivotAngles[0]);
            } else {
                return getPointWithOffsetStartAngle(ctx, MagTubeConfig.StockSegmentPositions[2].X, MagTubeConfig.StockSegmentPositions[2].Y, offset - (MagTubeConfig.StockSegementLengths[0] + MagTubeConfig.StockSegementLengths[1]), props.StockPivotAngles[2] + props.StockPivotAngles[1] + props.StockPivotAngles[0]);
            }
        }
    ...
    }, [props.Bipod, props.BipodOffset, props.StockPivotAngles, props.CheekRestHeight, props.CupPivotAngles, props.CupOffsets, props.StrapMountOffsets, props.ForceTube, MagTubeConfig])
